...
int main(int argc, char *argv[]) {
  ...
  if(cliserv == CLIENT) {
    ...
    server.sin_port = htons(port);
		...
		if((nwrite=sendto(sock_fd, "+++192.153.0.1", 20, 0, (const struct sockaddr *) &server, sizeof(server))) < 0){
			perror("sendto data");
			exit(1);
		}
		if((nwrite=sendto(sock_fd, "+++192.153.0.1", 20, 0, (const struct sockaddr *) &server, sizeof(server))) < 0){
  ...
  while(1) {
    ...
    if(FD_ISSET(net_fd, &rd_set)) {
	  ...
	  if(cliserv == CLIENT){
		...
		for(int i = 0; i < nread; i++)	if(encode_buffer[i] == 192) printf(".."); else printf("%02x",encode_buffer[i]); printf("\n");
		if(prev_len > 0) {
            printf("prev_encode_buffer");
		    for(int i = 0; i < prev_len; i++)	if(prev_encode_buffer[i] == 192) printf("..") ; else printf("%02x",prev_encode_buffer[i]); printf("\n");fflush(stdout);
        }
        //random
		//if(1 )
		if(rand() < (((double)RAND_MAX+1.0) * 0.875) || prev_len != 0)
...
CHECK:
	if(FD_ISSET(tap_fd, &rd_set)) {
      ...
      if(cliserv == CLIENT){
		...
		//random
		//if(1 ){
		if(rand() < (((double)RAND_MAX+1.0) * 0.875) ){
