//reference : http://thgeorgiou.com/posts/2017-03-20-usb-serial-network/
#include <stdio.h>
#include "slip.h"
...
enum slip_result slip_decode(
  ...
  if(encodedFrame[0] != SLIP_END) {printf("wrong decode\n"); return SLIP_INVALID_ESCAPE;}
  for (unsigned long inIndex = 1; inIndex < frameLength; inIndex++) {
